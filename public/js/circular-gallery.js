// Circular Gallery with OGL
(function() {
  'use strict';

  // Check if OGL is loaded
  if (typeof OGL === 'undefined') {
    console.error('OGL library not loaded. Please include OGL before this script.');
    return;
  }
  ...
  class Media {
    ...
    onResize({ screen, viewport } = {}) {
      ...
      this.plane.scale.x = (this.viewport.width * (675 * this.scale)) / this.screen.width;
  ...
  class CircularGalleryApp {
    constructor(container, options = {}) {
      ...
      this.font = options.font || 'bold 24px sans-serif';
      this.scrollSpeed = options.scrollSpeed || 1.5;
      this.scrollEase = options.scrollEase || 0.08;
      this.autoPlaySpeed = options.autoPlaySpeed || 0.5;
    ...
    update() {
      if (!this.isUserInteracting && !this.isDown) {
        this.scroll.target += this.autoPlaySpeed * 0.02;
      }
  ...
  function initGallery() {
    ...
    if (!galleryContainer) {
      console.warn('Circular gallery container not found');
      return;
    }

    const imageCount = parseInt(galleryContainer.dataset.imageCount) || 12;
