    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-sm text-[color:var(--muted)] mt-1">Total {{ $subCategories->total() }} sub kategori terdaftar
                </p>
            </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-3xl font-bold text-[color:var(--text-dark)] mt-1">
                            {{ $subCategories->unique('category_id')->count() }}</h3>
                    </div>
                    <div
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-3xl font-bold text-[color:var(--text-dark)] mt-1">
                            {{ $subCategories->sum('articles_count') }}</h3>
                    </div>
                    <div
                        ...
                        @forelse($subCategories as $subCategory)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            @else
                                                <p class="text-xs text-[color:var(--muted)] lg:hidden mt-0.5">
                                                    <i
                                                        class="fa fa-folder text-xs mr-1"></i>{{ $subCategory->category->name }}
                                                </p>
                                            @endif
                                        </div>
                                    </div>
                                ...
                                <td class="px-6 py-4 hidden md:table-cell">
                                    <code
                                        class="px-2 py-1 bg-gray-100 text-[color:var(--text-dark)] text-xs rounded font-mono">{{ $subCategory->slug }}</code>
                                </td>
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-lg font-semibold text-[color:var(--text-dark)] mb-2">Belum ada sub
                                            kategori</p>
                                        <p class="text-sm text-[color:var(--muted)] mb-4">Mulai dengan menambahkan sub
                ...
                <div class="px-6 py-4 border-t border-gray-100 bg-gray-50">
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div class="text-sm text-[color:var(--muted)]">
                            Menampilkan <span
                                class="font-semibold text-[color:var(--text-dark)]">{{ $subCategories->firstItem() }}</span>
                            - <span
                                class="font-semibold text-[color:var(--text-dark)]">{{ $subCategories->lastItem() }}</span>
                            dari <span
                                class="font-semibold text-[color:var(--text-dark)]">{{ $subCategories->total() }}</span>
                            data
                        </div>
        ...
        <script>
            ...
            document.getElementById('searchInput').addEventListener('input', function(e) {
                ...
                tableRows.forEach(row => {
                    ...
                    if (subCategoryName.includes(searchTerm) || categoryName.includes(searchTerm) || slug
                        .includes(searchTerm)) {
                        row.style.display = '';
                    } else {
