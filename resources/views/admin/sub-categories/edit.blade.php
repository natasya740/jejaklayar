...
@section('page-subtitle', 'Perbarui informasi sub kategori')
        ...
        <ol class="inline-flex items-center space-x-2 text-sm">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('admin.sub-categories.show', $subCategory) }}" class="text-[color:var(--muted)] hover:text-[color:var(--yellow-2)] transition-colors">
                        {{ $subCategory->name }}
                    </a>
                </div>
            </li>
    ...
    <div class="bg-gradient-to-r from-amber-400 to-amber-500 rounded-xl shadow-lg p-6 mb-6">
        <div class="flex items-center gap-4">
            ...
            <div class="text-white">
                ...
                <p class="text-white/80">Perbarui informasi: {{ $subCategory->name }}</p>
            </div>
        </div>
    </div>
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="bg-amber-50 border border-amber-200 rounded-lg p-4">
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex-1">
                            ...
                            <code class="text-sm bg-white px-3 py-1.5 rounded border border-amber-200 text-amber-900">{{ $subCategory->slug }}</code>
                        </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all appearance-none bg-white @error('category_id') border-red-500 @enderror" 
                                id="category_id" 
                                name="category_id" 
                                required>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <div class="absolute bottom-3 right-3 text-xs text-[color:var(--muted)]">
                            <span id="charCount">{{ strlen($subCategory->description ?? '') }}</span> / 500
                        </div>
                    </div>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">
                    ...
                    <div class="flex gap-3">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-orange-800">
                                Sub kategori ini memiliki <strong>{{ $subCategory->articles->count() }} artikel</strong>. 
                                Perubahan nama atau kategori induk akan mempengaruhi URL artikel yang terkait.
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="flex gap-3">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-sm font-semibold text-[color:var(--text-dark)]">{{ $subCategory->created_at->format('d F Y, H:i') }}</p>
                    <p class="text-xs text-[color:var(--muted)] mt-0.5">{{ $subCategory->created_at->diffForHumans() }}</p>
                </div>
            </div>
            <div class="flex gap-3">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-sm font-semibold text-[color:var(--text-dark)]">{{ $subCategory->updated_at->format('d F Y, H:i') }}</p>
                    <p class="text-xs text-[color:var(--muted)] mt-0.5">{{ $subCategory->updated_at->diffForHumans() }}</p>
                </div>
            </div>
        </div>
    </div>
