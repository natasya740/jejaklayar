        ...
        <ol class="inline-flex items-center space-x-2 text-sm">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <span class="text-[color:var(--text-dark)] font-medium">{{ $category->name }}</span>
                </div>
            </li>
        </ol>
    </nav>
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div class="flex items-center gap-4">
                <div class="w-16 h-16 bg-white bg-opacity-20 backdrop-blur-sm rounded-xl flex items-center justify-center shadow-lg">
                    <i class="fa fa-folder text-3xl text-white"></i>
                </div>
                <div class="text-white">
                    <h2 class="text-2xl font-bold mb-1">{{ $category->name }}</h2>
                    <p class="text-white/80">Detail lengkap kategori</p>
                </div>
            </div>
        ...
        <div class="lg:col-span-2 space-y-6">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 text-sm font-medium text-[color:var(--muted)] w-1/3">
                                    <i class="fa fa-tag mr-2"></i>Nama Kategori
                                </td>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <code class="px-3 py-1.5 bg-gray-100 text-[color:var(--text-dark)] text-sm rounded-lg font-mono">{{ $category->slug }}</code>
                                </td>
                            </tr>
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 text-sm font-medium text-[color:var(--muted)]">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-sm text-[color:var(--text-dark)]">
                                    ...
                                    <span class="text-[color:var(--muted)] text-xs ml-2">({{ $category->created_at->diffForHumans() }})</span>
                                </td>
                            </tr>
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 text-sm font-medium text-[color:var(--muted)]">
                                ...
                                <td class="px-6 py-4 text-sm text-[color:var(--text-dark)]">
                                    ...
                                    <span class="text-[color:var(--muted)] text-xs ml-2">({{ $category->updated_at->diffForHumans() }})</span>
                                </td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg hover:shadow-xl transition-shadow duration-300">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            ...
                            <h3 class="text-4xl font-bold">{{ $category->subCategories->count() }}</h3>
                        </div>
                        <div class="w-14 h-14 bg-white/20 rounded-xl flex items-center justify-center">
                            <i class="fa fa-folder-open text-3xl"></i>
                        </div>
                    </div>
                    <p class="text-white/70 text-sm">Total sub kategori aktif</p>
                </div>
                ...
                <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white shadow-lg hover:shadow-xl transition-shadow duration-300">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <p class="text-white/80 text-sm mb-1">Artikel</p>
                            <h3 class="text-4xl font-bold">{{ $category->articles->count() }}</h3>
                        </div>
                        <div class="w-14 h-14 bg-white/20 rounded-xl flex items-center justify-center">
                            <i class="fa fa-file-alt text-3xl"></i>
                        </div>
                    </div>
                    <p class="text-white/70 text-sm">Total artikel terpublikasi</p>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-xl shadow-[var(--shadow-sm)] overflow-hidden">
                ...
                <div class="p-4">
                    @forelse($category->subCategories as $subCategory)
                        <div class="flex justify-between items-center p-3 mb-2 bg-gray-50 rounded-lg hover:bg-gray-100 hover:shadow-md transition-all duration-200 group">
                            <div class="flex items-center gap-2 flex-1 min-w-0">
                                <i class="fa fa-folder text-blue-600"></i>
                                <span class="text-sm font-semibold text-[color:var(--text-dark)] truncate">{{ $subCategory->name }}</span>
                            </div>
                            <a href="{{ route('admin.sub-categories.show', $subCategory) }}" 
                               class="flex-shrink-0 w-8 h-8 flex items-center justify-center bg-blue-100 text-blue-600 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-blue-200 transition-all duration-200"
                               title="Lihat Detail">
                    ...
                    @empty
                        <div class="text-center py-8">
                            ...
                            <p class="text-sm text-[color:var(--muted)] mb-3">Belum ada sub kategori</p>
                ...
                <div class="p-4">
                    @forelse($category->articles->take(5) as $article)
                        <a href="{{ route('admin.articles.show', $article) }}" 
                           class="block p-3 mb-3 rounded-lg hover:bg-gray-50 hover:shadow-md transition-all duration-200 group">
                ...
                <div class="space-y-2">
                    <a href="{{ route('admin.categories.edit', $category) }}" 
                       class="flex items-center gap-3 p-3 bg-white rounded-lg hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                        ...
                        <span class="text-sm font-semibold text-[color:var(--text-dark)]">Edit Kategori</span>
                    </a>
                    <a href="{{ route('admin.sub-categories.create') }}" 
                       class="flex items-center gap-3 p-3 bg-white rounded-lg hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                        ...
                        <span class="text-sm font-semibold text-[color:var(--text-dark)]">Tambah Sub Kategori</span>
                    </a>
                    <a href="{{ route('admin.articles.create') }}" 
                       class="flex items-center gap-3 p-3 bg-white rounded-lg hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                    ...
                    <form action="{{ route('admin.categories.destroy', $category) }}" 
                          method="POST" 
                          onsubmit="return confirm('Yakin ingin menghapus kategori ini beserta semua sub kategori dan artikel yang terkait?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" 
                            ...
                            <span class="text-sm font-semibold text-red-600">Hapus Kategori</span>
                        </button>
                    </form>
                </div>
            </div>
